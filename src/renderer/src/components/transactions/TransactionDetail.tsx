...
export function TransactionDetail({
  ...
  async function handleSaveAmount(): Promise<void> {
    ...
    try {
      ...
      await window.api.transactions.update(transactionId, { amount: parsed });
      setTransaction((prev) => prev ? { ...prev, amount: parsed } : prev);
      emitDataChange("transactions");
    } catch (err) {
  ...
  async function handleToggleSign(): Promise<void> {
    if (!transaction) return;
    const newAmount = -Number(transaction.amount);
    const newType = transaction.type === "deposit" ? "withdrawal" : "deposit";
    try {
      await window.api.transactions.update(transactionId, { amount: newAmount, type: newType });
      setTransaction((prev) => prev ? { ...prev, amount: newAmount, type: newType } : prev);
      emitDataChange("transactions");
    } catch (err) {
        ...
        <div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <button
              ...
              onMouseEnter={(e) => { e.currentTarget.style.background = "#0e639c"; e.currentTarget.style.borderColor = "#0e639c"; e.currentTarget.style.color = "#fff"; }}
              onMouseLeave={(e) => { e.currentTarget.style.background = "#333"; e.currentTarget.style.borderColor = "#555"; e.currentTarget.style.color = "#ccc"; }}
            >
              Flip ±
            </button>
          </div>
      ...
      >
        {detailRow(
          ...
          editingAmount ? (
            ...
            <span
              onClick={() => {
                setAmountInput(Number(transaction.amount).toFixed(2));
